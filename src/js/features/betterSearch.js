(() => {
    ...
    const HOVER_CARD_SELECTOR = [
        '[data-t="hover-component"]', '[data-testid="hover-component"]',
        '[class^="search-show-card-hover--"]', '[class^="browse-card-hover--"]'
    ].join(", ");
    const AVAILABILITY_SELECTOR = '[data-cr-toolkit="availability-labels"]';
    ...
    let useAvailabilityIcons = false;
    let availabilityTooltip = null;
    let availabilityTooltipTarget = null;
    ...
    function initBetterSearch() {
        if (!initialized) {
            ...
            window.addEventListener("resize", hideAvailabilityTooltip);
            document.addEventListener("keydown", event => {
                if (event.key === "Escape") hideAvailabilityTooltip();
            });
        }
    ...
    function observePage() {
        ...
        const observer = new MutationObserver(mutations => {
            if (availabilityTooltipTarget && !availabilityTooltipTarget.isConnected) {
                hideAvailabilityTooltip();
            }
            syncSearchPage();
            if (!betterSearchEnabled || !isSearchPage()) return;
            if (mutations.every(mutation => mutation.target.closest?.(
                '#cr-better-search, [data-cr-toolkit="availability-labels"], #cr-availability-tooltip'
            ))) return;
            syncSearchQuery();
            injectUI();
            scheduleApply();
    ...
    function handleBridgeMessage(event) {
        ...
        if (event.data.type === "CR_BETTER_SEARCH_ICONS") {
            const enabled = event.data.enabled === true;
            if (useAvailabilityIcons !== enabled) {
                hideAvailabilityTooltip();
                useAvailabilityIcons = enabled;
                scheduleApply();
            }
            return;
        }
    ...
    function applyFiltersToResults() {
        ...
        cards.forEach(card => {
            ...
            if (!matches && card.contains(availabilityTooltipTarget)) hideAvailabilityTooltip();
        });
    }
    ...
    function clearBetterSearchEffects() {
        ...
        decoratedCards.forEach((target, card) => {
            target.removeAttribute("data-cr-search-hidden");
            card.querySelectorAll(AVAILABILITY_SELECTOR).forEach(label => label.remove());
    ...
    function applyCalendarFilters() {
        ...
        cards.forEach(card => {
            ...
            if (!visible && release.contains(availabilityTooltipTarget)) hideAvailabilityTooltip();
            if (visible) shown++;
        });
    ...
    function updateAvailabilityLabels(card, availability, labels = getAvailabilityLabels(availability)) {
        updateAvailabilityContainer(card, labels, false);
        // These overlays cover both the normal card and its labels. Render the same
        // statuses inside each overlay, including overlays mounted after the card.
        card.querySelectorAll(HOVER_CARD_SELECTOR).forEach(hover => {
            updateAvailabilityContainer(hover, useAvailabilityIcons ? labels : [], true);
        });
    }

    function updateAvailabilityContainer(card, labels, isHover) {
        const context = isHover ? "hover" : "card";
        let container = card.querySelector(
            `${AVAILABILITY_SELECTOR}[data-cr-availability-context="${context}"]`
        );

        if (!labels.length) {
            if (container?.contains(availabilityTooltipTarget)) hideAvailabilityTooltip();
            container?.remove();
            return;
        }
        ...
        if (!container) {
            ...
            container.dataset.crAvailabilityContext = context;
            ...
            const footer = card.querySelector(isHover
                ? '[class*="search-show-card-hover__meta--"], [class*="browse-card-hover__meta--"]'
                : card.matches('.release')
                ? '.availability'
                : '[class*="search-show-card__footer"], [class*="browse-card__footer"]');
            const body = card.querySelector(isHover
                ? '[class*="search-show-card-hover__body--"], [class*="browse-card-hover__body--"]'
                : '[class*="search-show-card__body"], [class*="browse-card__body"]');
            if (isHover) {
                // The overlay's full-card navigation link has z-index: 2.
                // Inherit pointer-events so hidden genre overlays stay inert.
        ...
        if (container.contains(availabilityTooltipTarget)) hideAvailabilityTooltip();
        container.style.flexDirection = useAvailabilityIcons ? "row" : "column";
        container.style.gap = useAvailabilityIcons ? "8px" : "2px";
        container.style.width = useAvailabilityIcons ? "max-content" : "100%";
        container.style.maxWidth = "100%";
        container.replaceChildren();

        labels.forEach(label => {
            const element = createAvailabilityLabel(label.text, label.matches, label.type);
    ...
    function createAvailabilityLabel(text, matches, type) {
        const label = document.createElement("span");
        if (useAvailabilityIcons) {
            label.setAttribute("role", "img");
            label.setAttribute("aria-label", text);
            label.tabIndex = 0;
            label.style.display = "inline-flex";
            label.style.cursor = "default";
            // Avoid native title tooltips: the calendar's qTip preview handles
            // hover separately and can suppress them.
            label.addEventListener("pointerenter", () => showAvailabilityTooltip(label, text));
            label.addEventListener("focus", () => showAvailabilityTooltip(label, text));
            label.addEventListener("pointerleave", hideAvailabilityTooltip);
            label.addEventListener("blur", hideAvailabilityTooltip);
            label.appendChild(createAvailabilityIcon(matches === null ? "no_data_provided" : type));
        } else {
    ...
    function showAvailabilityTooltip(label, text) {
        hideAvailabilityTooltip();
        if (!useAvailabilityIcons || !betterSearchEnabled || !label.isConnected) return;
        const tooltip = document.createElement("div");
        tooltip.id = "cr-availability-tooltip";
        tooltip.dataset.crToolkit = "availability-tooltip";
        tooltip.setAttribute("role", "tooltip");
        tooltip.textContent = text;
        // Mount outside cards, carousels and the calendar's scrolling viewport.
        // The high stacking level also keeps it above the legacy qTip preview.
        Object.assign(tooltip.style, {
            position: "fixed", zIndex: "2147483647", pointerEvents: "none",
            boxSizing: "border-box", width: "max-content", maxWidth: "min(320px, calc(100vw - 16px))",
            padding: "8px 10px", border: "1px solid #59595b", borderRadius: "4px",
            background: "#23252b", color: "#ffffff", font: "12px/1.5 Arial, sans-serif",
            whiteSpace: "normal", overflowWrap: "anywhere", boxShadow: "0 4px 12px #0006"
        });
        document.body.appendChild(tooltip);
        availabilityTooltip = tooltip;
        availabilityTooltipTarget = label;
    ...
    function hideAvailabilityTooltip() {
        availabilityTooltipTarget?.removeAttribute("aria-describedby");
        availabilityTooltipTarget = null;
        availabilityTooltip?.remove();
        availabilityTooltip = null;
    }

    function createAvailabilityIcon(type) {
        const definition = AVAILABILITY_ICONS[type];
        const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
        ...
        definition.paths.forEach(attributes => {
            const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
